function sigma = responseSpectDeviationCostFunction(target, w, Fs, nPoles,NFIR, params, freqWeightFlag, levelWeightFlag)
...
if freqWeightFlag
    ...
    sigma = spectrumDeviation(db(target),db(filterResp),Weight);             %no weight
...
sigma = spectrumDeviation(db(target),db(filterResp));             %no weight
end
