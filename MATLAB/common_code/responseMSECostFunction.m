function MSE = responseMSECostFunction(target, w, Fs, nPoles, NFIR, params, freqWeightFlag, levelWeightFlag)
...
if freqWeightFlag
    ...
    MSE = weightedResponseMSE(db(target),db(filterResp),Weight);
...
MSE = weightedResponseMSE(db(target),db(filterResp),1);             %no weight
end
