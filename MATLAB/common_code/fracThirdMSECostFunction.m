function MSE = fracThirdMSECostFunction(target, w, Fs, nPoles, NFIR, params,freqWeightFlag, levelWeightFlag)
...
if freqWeightFlag
    ...
    Weight = fracOctAnalysis(w,ht,Fs,'third',false);
    Weight = Weight(:,2);
    if levelWeightFlag.Flag
        ...
        expw = fracOctAnalysis(w,expw,Fs,'third',false);
        expw = expw(:,2);
        Weight = Weight.*expw;
    end
    MSE = fracOctMSE(db(target),db(filterResp),'third',w,Fs,Weight); 
else if levelWeightFlag.Flag
    expw = expanderWeight(db(target),levelWeightFlag.Threshold,levelWeightFlag.Ratio,levelWeightFlag.KneeWidth);                   %level weight
    Weight = fracOctAnalysis(w,expw,Fs,'third',false);
    Weight = Weight(:,2);
    MSE = fracOctMSE(db(target),db(filterResp),'third',w,Fs,Weight); 
else
  MSE = fracOctMSE(db(target),db(filterResp),'third',w,Fs);  
end
